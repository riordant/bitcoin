...
bool Recover::askRecover(bool& newWallet)
{
    ...
    bool regTest = gArgs.GetBoolArg("-regtest", false);
    bool testNet = gArgs.GetBoolArg("-testnet", false);
        ...
        while(true)
        {
            if(!recover.exec())
            {
                ...
                if(recover.ui->recoverExisting->isChecked()) {
                    ...
                    gArgs.SoftSetArg("-mnemonic", mnemonic);
                }
                ...
                if(recover.ui->usePassphrase->isChecked()) {
                    ...
                    gArgs.SoftSetArg("-mnemonicpassphrase", mnemonicPassPhrase);
                }

                if(use12)
                    gArgs.SoftSetBoolArg("-use12", true);
                break;
            }
        }
    }
